            ...
            <div class="logout px-4">
                <a class="text-decoration-none text-dark d-flex align-items-center" href="">
                    <div style="width: 30px;height: 30px;">
                        <img class="w-100 h-100" src="{{ asset('assets\image\user_default.png') }}" alt="">
                    </div>
                    <div class="fw-bold ms-3 name_user">
                        @php
                            $userName = auth()->user()->name;
                            $userNameParts = explode(' ', $userName);
                            $numParts = count($userNameParts);
                            if ($numParts == 1) {
                                echo $userName;
                            } else {
                                echo implode(' ', array_slice($userNameParts, 2));
                            }
                        @endphp
                    </div>
                </a>
                <div class="select_logout" >
                    <ul>
                        <li class="py-2 ps-3">
                            <a class="text-decoration-none text-dark " href="{{ route('tai-khoan') }}">
                                <i class="fa-solid fa-clipboard-user"></i>
                                <span class="ms-3">Tài khoản</span>
                            </a>
                        </li>
                        <li class="py-2 ps-3">
                            <a class="text-decoration-none text-dark " href="{{ route('lich-su-giao-dịch') }}">
                                <i class="fa-solid fa-list-ol"></i>
                                <span class="ms-3">Lịch sử</span>
                            </a>
                        </li>
                        <li class="py-2 ps-3">
